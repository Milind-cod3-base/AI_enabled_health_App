...
# checking and/or creating if tasks table doesnt exist in
# motivationTasks.db file
...
# for the task manager database query, along with one more parameter-
# movement profile (given by AI)
def taskQuery(username):
    ...
    cur.execute(
        "SELECT job, age, gender FROM data WHERE username=?",
        [username])
...
# this will get the classified movemnt profile from the output and store
# it into the db file
...
def queryGraph(profile, lowerLimit, upperLimit):
    ...
    # items will give a list which contains timestamp and profile in the form
    # of tuples
    items = c.fetchall()
    ...
    # returning here because it will end the function and connection to db can
    # close before this
    return items


# a function to get the 4 tasks from the task manager based on the user's
# job, age, gender, profile
def getTask():

    # storing job, age and gender to the variables of the current user and its
    # profile
...
# CAUTION: clear table function, it will delete the mentioned table, use
# with caution
